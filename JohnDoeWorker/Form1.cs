...
namespace JohnDoeWorker
{
    public partial class Form1 : Form
    {
        ...
        //sql tables
        string dataTable = "[dbo].[AllWorkers.JohnDoe_Data]";
        string doneDataTable = "[dbo].[AllWorkers.JohnDoe_Done]";
                ...
                using (selectCMD = new SqlCommand(selectSQL, cnSQL))
                {
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            txtAccount.Text = dr["AccountNumber"].ToString();
                            txtFirstName.Text = dr["PatientFirstName"].ToString();
                            txtLastName.Text = dr["PatientLastName"].ToString();
                            txtPatientName.Text = dr["PatientFullName"].ToString();
                        }
                    }
        ...
        void GetRemainingTotals()
        {
            ...
            using (cnSQL = new SqlConnection(sqlConnStr))
            {
                ...
                using (selectCMD = new SqlCommand(selectSQL, cnSQL))
                {
                    ...
                    //else there are no records in the table and we will show 0
                    else
                    {
                        lblRemainingTotalCount.Text = "0";
                    }
                    selectCMD.Parameters.Clear();
                }
            }
        ...
        void StoreToDoneTable()
        {
            ...
            (DebtorNumber, AccountNumber, PatientFirstName, PatientLastName, PatientFullName, WorkerTimestamp, CurrentUser)
            ...
            using (cnSQL = new SqlConnection(sqlConnStr))
            {
                ...
                using (insertCMD = new SqlCommand(insertSQL, cnSQL))
                {
                    ...
                    insertCMD.Parameters.Add("@AccountNumber", SqlDbType.VarChar).Value = txtAccount.Text;
                    insertCMD.Parameters.Add("@PatientFirstName", SqlDbType.VarChar).Value = txtFirstName.Text;
                    insertCMD.Parameters.Add("@PatientLastName", SqlDbType.VarChar).Value = txtLastName.Text;
                    insertCMD.Parameters.Add("@PatientFullName", SqlDbType.VarChar).Value = txtPatientName.Text;
